...
; But the sorter wants a list: so we activate it like this (merge-sort (list 3 2 1 6 5 4 9 8 7)).
    ...
    (if (exactly-one? list-of-lists) 
        ...
        (director 
          (feeder list-of-lists))))) 
                     ...
                     ; A list with exactly one item will become empty if we remove the first item.
                     ; So we remove the first item, and then ask if it is empty.
(define exactly-one? 
  (lambda (ls) 
    (if (empty? (without-first ls))
        yes 
        no))) 
